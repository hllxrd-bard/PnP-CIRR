...
def patch_app_js(path: Path) -> None:
    ...
    if combined_listener not in text:
        text = replace_once(
            text,
            direct_listener,
            combined_listener,
            "composition mode listener",
        )

    vlm_listener = '$("use-vlm").addEventListener("change", updateVlmControls);\n'
    if vlm_listener not in text:
        text = replace_once(
            text,
            combined_listener,
            combined_listener + vlm_listener,
            "combined composition mode listener",
        )

    initial_block = "updateCompositionMode();\nupdateVlmControls();\n"
    if initial_block not in text:
        stripped = text.rstrip()
        if not stripped.endswith("updateCompositionMode();"):
            raise RuntimeError(
                "Could not find the final updateCompositionMode() initialization call."
            )
        stripped = stripped[: -len("updateCompositionMode();")]
        text = stripped + initial_block
