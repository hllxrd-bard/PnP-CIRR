...
ZIP_ARG="${1:-}"
if [[ -z "$ZIP_ARG" ]]; then
  if [[ -f "$ROOT_DIR/pnp_cirr_vlm_multi_provider_patch.zip" ]]; then
    ZIP_ARG="$ROOT_DIR/pnp_cirr_vlm_multi_provider_patch.zip"
  else
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    if [[ -f "$SCRIPT_DIR/pnp_cirr_vlm_multi_provider_patch.zip" ]]; then
      ZIP_ARG="$SCRIPT_DIR/pnp_cirr_vlm_multi_provider_patch.zip"
    else
      echo "Usage: $0 /path/to/pnp_cirr_vlm_multi_provider_patch.zip" >&2
      exit 2
    fi
  fi
fi
...
ZIP_TARGET="$TMP_DIR/pnp_cirr_vlm_multi_provider_patch.zip"
if [[ "$ZIP_SOURCE" != "$ZIP_TARGET" ]]; then
  mv -f "$ZIP_SOURCE" "$ZIP_TARGET"
fi

STAMP="$(date +%Y%m%d_%H%M%S)"
EXTRACT_DIR="$TMP_DIR/pnp_cirr_vlm_multi_provider_patch_$STAMP"
...
PAYLOAD_DIR="$EXTRACT_DIR/pnp_cirr_vlm_multi_provider_patch"
...
echo
echo "========== VLM MULTI-PROVIDER PATCH DONE =========="
echo "ZIP:      $ZIP_TARGET"
