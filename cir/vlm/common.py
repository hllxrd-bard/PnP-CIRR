...
def image_to_profile_data_url(
    image_path: str | Path,
    profile: dict[str, Any],
) -> str:
    """
    Resize/compress ảnh khi profile có image_max_side.

    Không có image_max_side hoặc giá trị <= 0:
        gửi nguyên ảnh như hành vi cũ.
    """
    max_side = int(profile.get("image_max_side") or 0)

    if max_side <= 0:
        return image_to_data_url(image_path)

    jpeg_quality = int(profile.get("image_jpeg_quality", 82))
    ...
    with Image.open(path) as image:
        ...
        # thumbnail giữ aspect ratio và không upscale ảnh nhỏ.
        image.thumbnail(
            (max_side, max_side),
